...
- Test case fails when batch event queue not cleared before testing in parallel
...
- Include vwo-log-messages as resource json files, instead of a separate dependency to resolve build issues that were ignoring this dependency in specific cases.
...
- Fix logger to check for universal {file} present in log message before trying to replace it
...
- Fixed Event Batching issue, where messages (impressions sent to VWO server) were getting dropped under high concurrent load.
...
- LoggerService changed to thread safe Singleton

## [1.42.0] - 2022-08-24

### Changed

- Log correctly when settings-file is not correct at the time os SDK initialiation
